...
def compute_node_reach_flows(
    solver: RiverNetworkPreissmannSolver,
    reach_states: Sequence[ReachState],
    reach_table: Optional[Sequence[Dict[str, Any]]] = None,
) -> List[Dict[str, Any]]:
    """
    列出每个节点与其相连河段的断面流量。
    流量符号与河段 start_node -> end_node 方向一致。
    """
    rows: List[Dict[str, Any]] = []
    for reach_idx, (reach, st) in enumerate(zip(solver.reaches, reach_states)):
        if reach_table is not None:
            rid = int(reach_table[reach_idx].get("reach_id", reach_idx + 1))
        else:
            rid = int(reach.data.reach_id)
        sn = int(reach.data.start_node)
        en = int(reach.data.end_node)
        rows.append(
            {
                "节点编号": sn,
                "reach_id": rid,
                "相连节点": en,
                "节点角色": "起点",
                "河段流量(m3/s)": round(float(st.q[0]), 4),
            }
        )
        rows.append(
            {
                ...
                "相连节点": sn,
                "节点角色": "终点",
                "河段流量(m3/s)": round(float(st.q[-1]), 4),
            }
        )
    rows.sort(key=lambda r: (r["节点编号"], r["reach_id"]))
    return rows
...
def _load_live_boundary_values() -> Dict[int, float]:
    """通过 app.py 实时获取三个边界节点当前时刻数据。"""
    course_dir = Path(__file__).resolve().parent.parent
    if str(course_dir) not in sys.path:
        sys.path.insert(0, str(course_dir))
    from app import fetch_boundary_node_values

    return fetch_boundary_node_values()
...
if __name__ == "__main__":
    ...
    # ---------------------------------------------------------
    # （三）边界条件：启动时读取实时值，作为当前时刻边界
    # ---------------------------------------------------------
    live_boundary_values = _load_live_boundary_values()
    ...
    print("模型初始化完毕，开始单时间步计算...")
    result = solver.solve(n_steps=n_steps, save_full_reach_history=False)

    # ---------------------------------------------------------
    # （四）结果导出：当前时刻各节点、各河段水位流量
    # ---------------------------------------------------------
    t_current = float(result["times"][-1])
    node_ids = result["node_ids"]
    node_heads = result["node_head_history"][-1]
    node_reach_flow_rows = compute_node_reach_flows(
        solver, result["final_reach_states"], reach_table_raw
    )
    flows_by_node: Dict[int, List[Dict[str, Any]]] = {}
    for row in node_reach_flow_rows:
        flows_by_node.setdefault(row["节点编号"], []).append(row)

    print(f"\n当前时刻 t = {t_current:.0f} s 各节点水位及相连河段流量：")
    node_rows: List[Dict[str, Any]] = []
    for nid in node_ids:
        node_idx = solver.id_to_index[nid]
        z_val = float(node_heads[node_idx])
        node_rows.append(
            {
                "节点编号": nid,
                "水位(m)": round(z_val, 4),
        ...
        print(f"  节点{nid}: 水位 = {z_val:.4f} m")
        for conn in flows_by_node.get(nid, []):
            q_val = conn["河段流量(m3/s)"]
            print(
                f"    河段{conn['reach_id']} "
                f"({conn['节点角色']}, 连节点{conn['相连节点']}): "
                f"流量 = {q_val:.4f} m3/s"
            )

    profile_rows: List[Dict[str, Any]] = []
    for reach_idx, (reach, st) in enumerate(zip(solver.reaches, result["final_reach_states"])):
            ...
            profile_rows.append(
                {
                    ...
                    "z_m": round(float(st.z[sec_idx]), 4),
                    "q_m3s": round(float(st.q[sec_idx]), 4),
                }
            )

    output_file = script_dir / "当前时刻各节点与河段水位流量.xlsx"
    print(f"\n正在保存当前时刻结果至 Excel...")
    with pd.ExcelWriter(output_file, engine="openpyxl") as writer:
        pd.DataFrame(node_rows).to_excel(writer, sheet_name="节点水位", index=False)
        pd.DataFrame(node_reach_flow_rows).to_excel(writer, sheet_name="节点河段流量", index=False)
        pd.DataFrame(profile_rows).to_excel(writer, sheet_name="河段", index=False)
